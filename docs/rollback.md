...
若新版升級 Redis 或改用 Valkey，回退還需要匹配的資料備份；**只換回舊映像不保證能讀取新版寫出的資料**。請先依 [Valkey 遷移與回復流程](valkey-migration.md#5-正式切換及回復) 停止寫入、保留新版資料，並準備舊映像與升級前資料副本，再執行下方映像切換步驟。
...
> 更換映像本身不會刪除 `/data` 掛載卷；但跨 Redis／Valkey 版本回退時，仍需還原相容的升級前備份。還原備份會回到備份當時的資料狀態，請先另存新版產生的設定與錄製資料。
